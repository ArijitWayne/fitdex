...
### 4.10 Phase 7 Boot & Update System
- **Startup Handoff**: Native Android splash hands off to the in-app boot sequence, then Home. The full warrior vignette is for cold or fresh application startup only; warm resume never replays it.
- **Loading Scope**: The full warrior boot is not a generic loading treatment. Ordinary in-app loading uses compact retro indicators that preserve page context and do not block unrelated work.
- **Faction Mapping**: Spartan boot uses teal armour, bronze/gold trim, crimson crest, and dark leather. Amazonian boot uses plum/crimson armour, gold trim, tiara, and ponytail. Both remain tied to the active theme family.
