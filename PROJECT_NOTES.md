...
## 40. Cross-App Page Frame and Navigation Consistency (Phase 9 In Progress)
...
## 47. GitHub Release Automation — Phase 4 Complete

FitDex release automation keeps version choice deliberate. Run one of `npm run release:prepare:patch`, `release:prepare:minor`, or `release:prepare:major`; shortcut commands `release:patch`, `release:minor`, and `release:major` run the same prepare stage. Prepare never publishes. It first requires branch `dev` and a clean working tree, computes the chosen SemVer change, increments Android `versionCode` exactly once, updates `package.json` and `package-lock.json`, runs lint, production build, and Capacitor Android validation, then makes the signed release APK and deterministic artifact/checksum. It writes ignored local `release-artifacts/release.json` and human-reviewed `release-notes.md` only after successful preparation.

Use `node scripts/prepare-release.mjs <patch|minor|major> --dry-run` for safe preflight. Dry-run validates code and prints planned version, Android code, tag, and filenames without changing version files, creating tags, calling GitHub, or uploading anything. `npm run release:prepare:first` is reserved for first public `v1.0.0`: it preserves semantic version and increments only Android `versionCode`. Test it first with `node scripts/prepare-release.mjs --first-release --dry-run`.

`npm run release:publish` is separate and explicit. It accepts only prepared version-file changes, verifies release metadata, APK/checksum presence and SHA-256, rejects an existing local or remote tag, checks GitHub CLI authentication, then commits prepared files, creates and pushes annotated tag, creates GitHub Release, uploads APK/checksum, and verifies release. `npm run release:publish -- --dry-run` validates prepared publish inputs without mutation. Never run publish until public-release approval.

Release metadata contract is stable for website sync and future in-app update work: `version`, `versionCode`, `tag`, `apk`, `apkDownloadUrl` (`null` until published), `checksum`, `sha256`, `releaseNotes`, and `publishedAt` (`null` until published). Release notes are generated as a draft with What’s New, Improvements, Fixes, and Notes sections; maintainers must review it rather than deriving user-facing copy from commits.

Environment-driven signing remains CI-compatible through `FITDEX_KEYSTORE_PATH`, `FITDEX_KEYSTORE_PASSWORD`, `FITDEX_KEY_ALIAS`, and `FITDEX_KEY_PASSWORD`. No secrets enter Git. Dry-run and static validation complete. Current repository state remains semantic version `1.0.0`, Android `versionCode 2`, with first public release intentionally deferred. Phase 4 release automation is complete.

## 48. Public Website — Phase 5 Production Foundation Implemented

The approved public-site direction is **Console**: Spartan Dark, teal technical grid, ivory type, rectangular framed controls, hard offset shadows, RPG telemetry labels, and real FitDex screenshots. Production site lives in `website/` as an independent React + TypeScript + Vite application. It is intended for Vercel with `website/` as project root; no Vercel project or deployment exists yet.

The existing FitDex PWA remains independently hosted on Cloudflare Workers Static Assets. Exercise media remains a separate Cloudflare Worker surface. Neither deployment nor PWA UI is modified by website work. Root `public/screenshots/` is canonical approved screenshot pack; matching unaltered copies in `website/public/screenshots/` make the Vercel build self-contained.

Website release UI is intentionally pre-launch and reserved for Phase 6 integration with Phase 4 release metadata: `version`, `versionCode`, `tag`, `apk`, `apkDownloadUrl`, `checksum`, `sha256`, `releaseNotes`, and `publishedAt`. Public `v1.0.0` remains unpublished; version stays `1.0.0` and Android `versionCode` stays `2`.

The production Console hero implements a desktop transient three-state hover stack: neutral, left hover, and right hover. Active Workout, Home, and Personal Records keep fixed identities; left or right hover compresses cards into a front/middle/back depth stack, while center hover restores neutral composition. Focus follows the same states and leaving the hero stack restores neutral. Mobile viewports (<= 768px) intentionally preserve one readable Home screenshot. Lower contextual screenshot panels (Train, Track, Level Up, and Exercise Dex; standalone bulk gallery removed for clarity) retain restrained hover and focus-visible depth treatment. Phase 5 is COMPLETE and live on Vercel at `https://fitdexinfo.vercel.app/`.

## 49. Release Hub & Automatic Release Sync — Phase 6 Implemented

Phase 6 implements the public FitDex Release Hub and automatic published-release synchronization within `website/`.

Key architectural principles and locked design decisions:

1. **Single Source of Truth**: Published GitHub Releases (`https://api.github.com/repos/ArijitWayne/fitdex/releases`) are authoritative. Ordinary git commits, documentation updates, and development refactors are strictly ignored.
2. **Release Data Layer**: Separated into `website/src/releases/` (`types.ts`, `releaseNotes.ts`, `normalize.ts`, `api.ts`). Normalizes GitHub release payloads into a stable `FitDexRelease` contract compatible with Phase 4 release artifacts, designed for clean reuse in Phase 7 (in-app updates).
3. **Release Filtering & Sorting**: Ignores drafts (`draft === true`) and prereleases (`prerelease === true`). Orders releases chronologically descending by `published_at` (newest first).
4. **Asset & Checksum Detection**: Deterministically detects the signed APK (`fitdex.<version>.apk`) and extracts SHA-256 digests from release body or `.sha256` asset metadata without fabricating values.
5. **Release Notes Parser**: Conservative markdown parser recognizing `NEW`, `IMPROVED`, and `FIXED` categories with safe fallback to `OTHER` to prevent data loss. Does not use unsafe HTML injection sinks.
6. **Current Real Pre-Release State**: Because FitDex public v1.0.0 is not yet published on GitHub, the live site shows a clean pre-release notice on `/` ("PUBLIC RELEASE COMING SOON") and an empty archive state on `/changelog` ("NO PUBLIC RELEASES YET."). No mock or demo releases exist in production.
7. **Single-Column Archive Layout**: Locked single vertical chronological stream. The newest release receives `LATEST` badge and `id="latest"`, supporting `/changelog#latest` anchor jumps with smooth scroll and pulse highlight.
