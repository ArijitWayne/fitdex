...
assert.match(shell, /<button className="wordmark"/)
assert.match(shell, /useResolvedBrightness/)
assert.match(shell, /setBrightness\(resolvedBrightness === 'dark' \? 'light' : 'dark'\)/)
assert.match(shell, /<Settings size=\{21\}/)
const styles = fs.readFileSync('src/styles/app.css', 'utf8')
assert.match(styles, /\.wordmark-image \{[^}]*object-fit: contain/s)
assert.match(styles, /@media \(min-width: 600px\)\s*\{[\s\S]*?\.wordmark-image\s*\{[^}]*width:\s*auto;[^}]*max-width:\s*190px;[^}]*height:\s*52px;/)
const native = fs.readFileSync('src/branding/nativeBranding.ts', 'utf8')
assert.match(native, /Capacitor\.isNativePlatform\(\)/)
assert.match(native, /export function supportsNativeAndroidLauncherBranding\(\)/)
assert.match(native, /Capacitor\.getPlatform\(\) === 'android'/)
assert.match(native, /Capacitor\.isPluginAvailable\('LauncherBranding'\)/)
assert.match(native, /!supportsNativeAndroidLauncherBranding\(\) \|\| activeFamily === family/)
assert.match(native, /activeFamily === family/)
assert.match(native, /setLauncherBranding\(\{ family \}\)/)
assert.match(native, /setLauncherBranding\(\{ family, relaunch: true \}\)/)

console.log('Branding assertions passed: family mapping, exact assets, responsive header, favicon replacement, and native LauncherBranding availability/idempotency guards')
