...
const { acknowledgeFirstUse, loadFirstUseGuidance } = await import('./firstUseGuidance.ts')
await db.open()
...
assert.deepEqual(await loadFirstUseGuidance(), { workoutLanding: true, nutritionSetup: 'skipped' })
await acknowledgeFirstUse('nutritionSetup', 'completed')
assert.equal((await loadFirstUseGuidance()).nutritionSetup, 'completed')
assert.equal((await db.settings.get('settings'))?.displayName, settings.displayName)
...
assert.match(workoutSession, /Rest Timer/)
assert.match(workoutSession, /Take a rest whenever you need one between sets/)
...
assert.match(onboarding, /section: 'Workout'/)
assert.match(onboarding, /section: 'Food'/)
assert.match(onboarding, /section: 'Weekly Plan & Streaks'/)
assert.match(onboarding, /section: 'Progress'/)
assert.match(onboarding, /section: 'Achievements'/)
assert.match(onboarding, /section: 'Battle Music'/)
assert.doesNotMatch(onboarding, /section: 'Personalization'/)
assert.match(onboarding, /Build Your Training/)
assert.match(onboarding, /Track Your Fuel/)
assert.match(onboarding, /Your Plan\. Your Campaign\./)
assert.match(onboarding, /Level Up With Facts/)
assert.match(onboarding, /Forge Your Legacy/)
assert.match(onboarding, /Focus Your Energy/)
...
assert.match(onboarding, /Mission Brief/)
assert.match(onboarding, /Choose Your Faction/)
assert.match(onboarding, /Choose Your Mode/)
assert.doesNotMatch(onboarding, /Set Your Brightness/)
assert.doesNotMatch(onboarding, /Steel • Petrol/)
assert.doesNotMatch(onboarding, /Plum • Amethyst/)
assert.match(onboarding, /Optional Nutrition Targets/)
assert.match(onboarding, /mission-actions-stacked/)
assert.match(onboarding, /mission-primary-btn/)
assert.match(onboarding, /mission-secondary-row/)
assert.match(onboarding, /saveNutritionTargets/)
assert.match(onboarding, /nutritionSetup.*skipped/)
...
assert.match(foodPage, /Log Food/)
...
assert.match(progress, /Total completed repetitions across all finished exercise sets/)
