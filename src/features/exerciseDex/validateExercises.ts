...
import { ACTIVE_FITDEX_EXERCISES, builtInExercises, BUILT_IN_EXERCISE_DATASET_VERSION, RETIRED_FITDEX_EXERCISE_SLUGS } from './exerciseData.ts'
...
for (const exercise of builtInExercises) {
  ...
  if (!exercise.sourceId || slugs.has(exercise.sourceId)) errors.push(`Missing or duplicate slug: ${exercise.name} / ${exercise.sourceId}`)
  ...
  ids.add(exercise.id)
  if (exercise.sourceId) slugs.add(exercise.sourceId)
...
console.log(`Exercise dataset valid: ${builtInExercises.length} FitDex canonical records, version ${BUILT_IN_EXERCISE_DATASET_VERSION}`)
console.log(counts)
