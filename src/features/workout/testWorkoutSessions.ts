...
assert.equal(first.workout.timerState, 'paused')
assert.equal(first.workout.lastResumedAt, undefined)
...
assert.equal(prepared.workout.timerState, 'paused')
assert.equal(prepared.workout.lastResumedAt, undefined)
assert.deepEqual(prepared.exercises.map((item) => item.exercise.exerciseId), [strength.id, custom.id])
assert.deepEqual(prepared.exercises.map((item) => item.sets.length), [3, 3])
await repository.discardWorkout(prepared.workout.id)

// Empty workouts cannot be completed and discard does not affect definitions.
const empty = await repository.startEmptyWorkout()
assert.equal(empty.workout.status, 'active')
assert.equal(empty.workout.timerState, 'paused')
assert.equal(empty.workout.accumulatedActiveSeconds, 0)
assert.equal(empty.workout.lastResumedAt, undefined)
await assert.rejects(() => repository.resumeWorkout(empty.workout.id), /Add at least one exercise to start your workout timer./)
...
// Defensive invariant: removing all exercises automatically pauses a running session
const autoPauseSession = await repository.startEmptyWorkout('Auto Pause Session')
await repository.addExercisesToWorkout(autoPauseSession.workout.id, [strength])
await repository.resumeWorkout(autoPauseSession.workout.id)
let autoPauseDetail = await repository.getWorkoutDetail(autoPauseSession.workout.id)
assert.equal(autoPauseDetail.workout.timerState, 'running')
await repository.removeWorkoutExercise(autoPauseDetail.exercises[0].exercise.id)
autoPauseDetail = await repository.getWorkoutDetail(autoPauseSession.workout.id)
...
db.close()
await Dexie.delete('fitdex')
console.log('Workout session tests passed: starts, snapshots, autosave, validation, active guard, custom logging, previous performance, completion, discard, zero-exercise timer guards, and immutable history')
