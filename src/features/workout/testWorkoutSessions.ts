...
await repository.updateWorkoutSet(firstSet.id, { weight: 50.5, reps: 10, completed: true })
...
// Completion preserves incomplete drafts without counting them as complete.
...
assert.equal(completed.exercises[0].sets.filter((set) => set.completed).length, 1)
...
assert.equal((await repository.listRecentWorkouts())[0].workout.id, first.workout.id)
...
await assert.rejects(() => repository.finishWorkout(empty.workout.id), /Complete at least one set/)
