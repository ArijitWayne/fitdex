...
// Removing every exercise from an active session leaves it active with zero exercises — no pause state to manage.
const clearedSession = await repository.startEmptyWorkout('Cleared Session')
await repository.addExercisesToWorkout(clearedSession.workout.id, [strength])
let clearedDetail = await repository.getWorkoutDetail(clearedSession.workout.id)
await repository.removeWorkoutExercise(clearedDetail.exercises[0].exercise.id)
clearedDetail = await repository.getWorkoutDetail(clearedSession.workout.id)
assert.equal(clearedDetail.exercises.length, 0)
assert.equal(clearedDetail.workout.status, 'active')
await repository.discardWorkout(clearedSession.workout.id)

db.close()
await Dexie.delete('fitdex')
console.log('Workout session tests passed: starts, snapshots, autosave, validation, active guard, custom logging, previous performance, completion, discard, zero-exercise handling, and immutable history')
