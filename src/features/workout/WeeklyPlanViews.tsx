...
export function WeeklyPlanEditor({ plan, routines, onChanged, onBack, onCreateRoutine }: { plan: WeeklyPlan; routines: readonly RoutineWithItems[]; onChanged: (plan: WeeklyPlan) => void; onBack: () => void; onCreateRoutine: () => void }) {
  ...
  const save = async (forceReset = false) => {
    try {
      setError('')
      const saved = await saveWeeklyPlan(days, { confirmReset: forceReset })
  ...
  if (showFirstUse) return <div className="page-stack workout-page"><ContextRail eyebrow="Weekly Plan" title="Your Weekly Plan defines streak obligations" footnote="Clears after viewing or setting plan" actions={<button className="primary-button" type="button" onClick={() => { playEffect('select'); void acknowledgeFirstUse('weeklyPlan'); setShowFirstUse(false) }}>Set My Plan</button>}><p>Routine and Workout days succeed only when completed as planned. Rest and No Plan preserve the streak without increasing it. Initial setup is free.</p></ContextRail></div>
  return <div className="page-stack workout-page"><Panel className="workout-flow-panel weekly-plan-editor"><div className="workout-flow-heading"><button className="dex-back-button" type="button" onClick={() => { playEffect('select'); onBack() }} aria-label="Back to Workout Hub"><ArrowLeft size={20} aria-hidden="true" /></button><div><p className="eyebrow">Recurring template</p><h2>Weekly Plan</h2><p>Choose an intention for each day. Routines are optional.</p></div></div><div className="weekly-plan-list">{WEEKDAY_IDS.map((day) => <button type="button" key={day} onClick={() => { playEffect('select'); setEditingDay(day) }}><strong>{WEEKDAY_LABELS[day].slice(0, 3)}</strong><span>{weeklyPlanAssignmentLabel(days[day], routines.map((entry) => entry.routine))}</span><ChevronRight size={18} aria-hidden="true" /></button>)}</div><p className="weekly-plan-commitment"><strong>Plan commitment</strong> Initial setup is free. One material schedule change per rolling 12 months protects your streak; later changes require confirmation and reset only the current Plan Streak.</p><button className="primary-button" type="button" onClick={() => void save()}>Save Weekly Plan</button>{error ? <p className="form-error" role="alert">{error}</p> : null}</Panel>
    ...
    {confirmReset ? <div className="workout-finish-backdrop"><section className="panel plan-change-dialog" role="alertdialog" aria-modal="true" aria-labelledby="plan-change-title"><p className="eyebrow">Plan commitment</p><h2 id="plan-change-title">Change Weekly Plan?</h2><p>You've already used your protected plan change for this rolling 12-month period. Changing your training schedule now will reset your current Plan Streak to 0.</p><p>This will not affect Lifetime XP, Level, Rank, Achievements, workout history, Personal Records, or Food history.</p><button className="secondary-button" type="button" autoFocus onClick={() => { playEffect('select'); setConfirmReset(false) }}>Keep Current Plan</button><button className="danger-button" type="button" onClick={() => void save(true)}>Change &amp; Reset Streak</button></section></div> : null}
  </div>
}
