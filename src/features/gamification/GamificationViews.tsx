import { ArrowLeft, CalendarClock, CircleHelp, ShieldCheck, Sparkles, X } from 'lucide-react'
...
export function StreakDetailView({ data, onBack, onChanged }: { data: GamificationDashboard; onBack: () => void; onChanged: (data: GamificationDashboard) => void }) {
  ...
  if (showFirstUse) return <div className="page-stack gamification-detail"><Subheader title="Plan Streak" onBack={onBack} /><ContextRail title="Three kinds of streak protection" footnote="Clears after real action, not page visit" actions={<button className="primary-button" type="button" onClick={() => { playEffect('select'); void acknowledgeFirstUse('streakProtection'); setShowFirstUse(false) }}>View My Streak</button>}><p>Freezes cover one missed planned day automatically. Travel / Sickness Pause protects 1–7 days without progress or Freeze use. Your first material plan change per rolling 12 months is protected.</p></ContextRail></div>
  return <div className="page-stack gamification-detail"><Subheader title="Plan Streak" onBack={onBack} /><Panel className="streak-hero" eyebrow="Current Plan Streak"><strong>{data.streak.current}</strong><span>Best · {data.streak.best}</span><small>successful planned training days</small></Panel><Panel eyebrow="This week"><ol className="streak-week">{week.map((snapshot, index) => { const date = shiftLocalDateKey(weekStart, index); return <li key={date}><strong>{new Intl.DateTimeFormat(undefined, { weekday: 'short' }).format(dateFromLocalDateKey(date)).toUpperCase()}</strong><span>{snapshotLabel(snapshot)}</span><em>{resultLabel(snapshot?.result)}</em></li> })}</ol></Panel><Panel eyebrow="Streak Freezes" title={`${data.freezeBalance} available`}><p>Earn +1 every 15 successful planned training days. Balance is unlimited. A Freeze automatically protects one missed planned training day without adding a streak day or XP.</p></Panel><Panel eyebrow="Travel / Sickness Pause" title="Protect an extended interruption"><p>Maximum {MAX_PAUSE_DAYS} days · {MAX_PAUSES_PER_ROLLING_YEAR} uses per rolling 12 months</p><strong>Pauses available: {data.pauseUsesRemaining} / {MAX_PAUSES_PER_ROLLING_YEAR}</strong><button className="secondary-button" type="button" disabled={!data.pauseUsesRemaining} onClick={() => { playEffect('select'); setPauseOpen(true) }}><CalendarClock size={17} aria-hidden="true" /> Plan a Pause</button></Panel>{pauseOpen ? <PauseDialog onClose={() => setPauseOpen(false)} onSaved={async () => { setPauseOpen(false); onChanged(await loadGamificationDashboard()) }} /> : null}</div>
}
