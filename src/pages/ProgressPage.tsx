...
function CharacterHero({ data, onRank, onAchievements }: { data: GamificationDashboard; onRank: () => void; onAchievements: () => void }) { const { playEffect } = useAudio(); const { progression } = data; const percent = progression.maxLevel ? 100 : Math.min(100, (progression.xpIntoLevel / progression.xpRequiredForNextLevel) * 100); return <Panel className="progress-character-hero"><GamificationBadge kind="rank" src={rankAssetPath(progression.rank)} label={`${progression.rank.name} rank emblem`} /><div className="progress-character-copy"><p className="eyebrow">Level {progression.level}</p><h2>{progression.rank.name}</h2><p>{progression.totalXp.toLocaleString()} Lifetime XP</p><div className="gamification-progress" role="progressbar" aria-label={progression.maxLevel ? 'Maximum level reached' : `Level ${progression.level} XP progress`} aria-valuemin={0} aria-valuemax={progression.maxLevel ? 100 : progression.xpRequiredForNextLevel} aria-valuenow={progression.maxLevel ? 100 : progression.xpIntoLevel}><i style={{ width: `${percent}%` }} /></div><small>{progression.maxLevel ? 'MAX LEVEL' : `${progression.xpIntoLevel.toLocaleString()} / ${progression.xpRequiredForNextLevel.toLocaleString()} XP to Level ${progression.level + 1}`}</small></div><div className="progress-character-actions"><button className="secondary-button" type="button" onClick={() => { playEffect('select'); onRank() }}>Rank Journey</button><button className="secondary-button" type="button" onClick={() => { playEffect('select'); onAchievements() }}>Achievements</button></div></Panel> }
function Attribute({ number, label, detail, value }: { number: string; label: string; detail: string; value: string }) { return <div className="progress-attribute"><b>{number}</b><span><strong>{label}</strong><small>{detail}</small></span><em>{value}</em></div> }
function SimpleBarChart({ buckets, valueLabel, emptyText }: { buckets: TrendBucket[]; valueLabel: string; emptyText: string }) { const maximum = Math.max(0, ...buckets.map((bucket) => bucket.value)); if (!buckets.length || maximum === 0) return <p className="progress-chart-empty">{emptyText}</p>; return <div className="progress-chart" role="img" aria-label={buckets.map((bucket) => `${bucket.label}: ${formatNumber(bucket.value)} ${valueLabel}`).join('; ')}>{buckets.map((bucket) => <div className="progress-chart-column" key={bucket.key} aria-hidden="true"><span className="progress-chart-value">{formatCompact(bucket.value)}</span><i style={{ '--progress-bar-height': `${Math.max(4, (bucket.value / maximum) * 100)}%` } as CSSProperties} /><small>{bucket.label}</small></div>)}</div> }
function PrPreview({ record, units }: { record: ExercisePersonalRecords; units: ProgressSourceData['units'] }) { const metric = record.metrics[0]; return <article><div><strong>{record.exerciseName}</strong><small>{metric.label} · {formatPersonalRecordDate(metric.dateKey)}</small></div><span><strong>{formatPersonalRecordMetric(metric, units)}</strong></span></article> }
function PersonalRecordsView({ records, units, onBack }: { records: ExercisePersonalRecords[]; units: ProgressSourceData['units']; onBack: () => void }) { const { playEffect } = useAudio(); const [query, setQuery] = useState(''); const filtered = useMemo(() => searchPersonalRecords(records, query), [records, query]); return <PageFrame className="page-stack progress-page personal-records-page"><header className="progress-subheader"><button className="back-button" type="button" aria-label="Back to Progress" onClick={() => { playEffect('select'); onBack() }}><ArrowLeft aria-hidden="true" /></button><div><p className="eyebrow">Progress archive</p><h1>Personal Records</h1><p>Your best logged performances</p></div></header><label className="progress-search"><Search size={19} aria-hidden="true" /><span className="sr-only">Search personal records by exercise name</span><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search exercises…" /></label><p className="progress-search-count" aria-live="polite">{filtered.length} {filtered.length === 1 ? 'exercise' : 'exercises'}</p>{filtered.length ? <section className="personal-record-list" aria-label="Exercise personal records">{filtered.map((record) => <article className="panel personal-record-card" key={record.exerciseId}><header><div><p className="eyebrow">{trackingLabel(record.trackingType)}</p><h2>{record.exerciseName}</h2></div><Award aria-hidden="true" /></header><dl>{record.metrics.map((metric) => <div key={metric.key}><dt>{metric.label}</dt><dd><strong>{formatPersonalRecordMetric(metric, units)}</strong><small>{formatPersonalRecordDate(metric.dateKey)}</small></dd></div>)}</dl><p>Last PR · {formatPersonalRecordDate(record.lastPrDateKey)}</p></article>)}</section> : <Panel className="progress-records-empty"><h2>{records.length ? 'No matching records' : 'No personal records yet'}</h2><p>{records.length ? 'Try another exercise name.' : 'Complete measurable workout sets to establish records.'}</p></Panel>}</PageFrame> }
function trackingLabel(type: ExercisePersonalRecords['trackingType']) { return type.replaceAll('_', ' ') }
