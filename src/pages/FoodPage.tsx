...
export function FoodPage({ onOpenSettings }: { onOpenSettings?: () => void }) {
  ...
  const navigateDate = (nextDate: string) => { playEffect('select'); setDate(nextDate); setEntries([]); setView({ kind: 'overview' }) }

  if (view.kind === 'add') return <FoodEditor date={date} meal={view.meal} remembered={view.remembered} editing={view.editing} onBack={() => { playEffect('select'); setView({ kind: 'meal', meal: view.meal }) }} onSaved={async () => { await refresh(); setView({ kind: 'meal', meal: view.meal }) }} />
  if (view.kind === 'meal') return <MealDetail date={date} meal={view.meal} entries={entries.filter((entry) => entry.meal === view.meal)} onBack={() => { playEffect('select'); setView({ kind: 'overview' }) }} onAdd={() => setView({ kind: 'add', meal: view.meal })} onEdit={(editing) => setView({ kind: 'add', meal: view.meal, editing })} onChanged={refresh} />
...
function FoodEditor({ date, meal, remembered, editing, onBack, onSaved }: { date: string; meal: FoodMeal; remembered?: RememberedFood; editing?: FoodLogEntry; onBack: () => void; onSaved: () => Promise<void> }) {
  ...
  useBackNavigation('food-editor', true, () => { if (stage === 'details' && !initial) setStage('suggestions'); else onBack() }, 20)
  ...
  const save = async () => { setError(''); setSaving(true); try { const draft: FoodDraft = { name, categoryId, customCategoryId }; for (const key of Object.keys(nutrition) as Array<keyof FoodNutrition>) draft[key] = parseOptionalNutrition(nutrition[key]); if (editing) await editFoodLog(editing.id, draft); else { const beforeCount = (await listMealEntries(date, meal)).length; await addFoodLog(date, meal, draft); const effect = foodSaveEffect(false, beforeCount); if (effect) playEffect(effect) } await onSaved() } catch (reason) { setError(reason instanceof Error ? reason.message : 'Unable to save food.') } finally { setSaving(false) } }
...
function CategoryDialog({ selectedId, selectedCustomId, onClose, onSelect }: { selectedId: PredefinedFoodCategoryId; selectedCustomId?: string; onClose: () => void; onSelect: (id: PredefinedFoodCategoryId, customId?: string, label?: string, color?: string) => void }) {
  ...
  const create = async () => { try { const beforeIds = new Set((await listCustomCategories()).map((category) => category.id)); const category = await createCustomCategory(name, color); if (!beforeIds.has(category.id)) playEffect('add'); onSelect('other', category.id, category.name, category.color) } catch (reason) { setError(reason instanceof Error ? reason.message : 'Unable to create category.') } }
