...
function Shot({ src, alt, label, className = '' }: ShotProps) {
  return (
    <figure className={`screen ${className}`} tabIndex={0}>
...
function App() {
  ...
  const releaseState = useReleases()
      ...
      <main id="main">
        {pathname === '/changelog' ? (
          ...
          <>
            ...
            <section className="value-strip" aria-label="FitDex principles">
              <div className="shell value-grid">
                {[
                  ['01', 'NO SUBSCRIPTION', 'Designed to stay free to use.'],
                  ['02', 'LOCAL-FIRST', 'Personal fitness history persists locally.'],
                  ['03', 'ANDROID-FIRST', 'Built for Android, with a live web app.'],
                  ['04', 'SOURCE-AVAILABLE', 'Core code uses PolyForm Noncommercial.'],
                ].map(([n, t, d]) => (
            ...
            <Feature
              reverse
              signal="NUTRITION & PROGRESS"
              title="TRACK WHAT COUNTS"
              text="Log food, review journal entries, watch progress, and keep personal records and consistency metrics in view."
              points={['Food and nutrition tracking', 'Journal for workouts and meals', 'Progress overview and personal records', 'Consistency metrics for long-term work']}
              shots={[['/screenshots/food.png', 'FitDex food tracking', 'FOOD // NUTRITION'], ['/screenshots/progress-records.png', 'FitDex personal records', 'PROGRESS // RECORDS']]}
            />
            <Feature
              signal="RPG PROGRESSION SYSTEM"
              title="LEVEL UP YOUR CONSISTENCY"
              text="Earn XP, pursue achievements, and keep training through plans, freezes, themes, and battle music."
              points={['52 permanent achievements and 9 named ranks', 'Weekly Plan and plan streak', 'Freezes for travel or sickness protection', 'Battle Music and theme selection']}
              shots={[['/screenshots/achievements.png', 'FitDex achievements', 'ACHIEVEMENTS // XP'], ['/screenshots/progress-overview.png', 'FitDex progress overview', 'PROGRESS // LEVELS']]}
            />

            <section className="section" id="dex">
            ...
            <section className="section alternate" id="local-first">
              <div className="shell privacy-grid">
                <div>
                  <p className="eyebrow">LOCAL-FIRST // USER CONTROL</p>
                  <h2>YOUR TRAINING<br /><em>STAYS YOURS.</em></h2>
                </div>
                <div>
                  <p className="lede">No mandatory account. Personal workout, nutrition, and progress data use local persistence.</p>
                  <p className="muted">Export and restore a user-controlled <code>.fitdex</code> backup. The app can still use network access for on-demand exercise media.</p>
                  <a className="text-link" href="https://github.com/ArijitWayne/fitdex/blob/dev/SECURITY.md" target="_blank" rel="noreferrer">READ SECURITY POLICY ↗</a>
                </div>
              </div>
            </section>
            ...
            <section className="section" id="why">
              <div className="shell why">
                <p className="eyebrow">WHY FITDEX // FOUNDER NOTE</p>
                <blockquote>Fitness and staying healthy should not require a recurring subscription.</blockquote>
                <p>FitDex combines training, nutrition, consistency, progress, and RPG motivation without requiring a subscription.</p>
                <p className="mission">“The goal isn&apos;t to replace discipline with gamification.<br />The goal is to make discipline easier to maintain.”</p>
                <p className="creator">ARIJIT BHADURI<br /><span>CREATOR &amp; LEAD DEVELOPER // FITDEX</span></p>
              </div>
            </section>
            ...
            <section className="section release-section" id="release-notes">
              {releaseState.status === 'loading' && (
                <div className="shell">
                  <div className="tactical-status-ticker">
                    ...
                    <span>SYNCING RELEASE DATA...</span>
                  </div>
                </div>
              )}
              ...
              {releaseState.status === 'error' && (
                <div className="shell">
                  <div className="tactical-status-ticker" style={{ borderColor: 'var(--danger)', color: '#ff9999' }}>
                    <span>RELEASE DATA FEED UNAVAILABLE // OPERATING IN LOCAL PRE-RELEASE MODE</span>
                    <button
                      className="button small"
                      type="button"
                      onClick={() => releaseState.reload()}
            ...
            <section className="section" id="download">
              <div className="shell platform-grid">
                ...
                <article>
                  ...
                  <h2>WEB APP</h2>
                  <p className="muted">Use FitDex PWA now.</p>
      ...
      <footer>
        <div className="shell footer-grid">
          <div>
            ...
            <small>Copyright © 2026 Arijit Bhaduri</small>
          </div>
          <nav aria-label="Footer">
            <a href={github} target="_blank" rel="noreferrer">GitHub</a>
            <a href={pwa} target="_blank" rel="noreferrer">Web App</a>
            <a
              href="/changelog"
              onClick={(e) => {
                e.preventDefault()
            ...
            <a
              href="#licensing"
              onClick={(e) => handleNavClick('#licensing', e)}
            >
              Licensing
            </a>
            <a href={`${github}/blob/dev/CONTRIBUTING.md`} target="_blank" rel="noreferrer">Contributing</a>
...
function Feature({ id, signal, title, text, points, shots, reverse }: FeatureProps) {
  return (
    <section className={`section ${reverse ? 'alternate' : ''}`} id={id}>
      <div className={`shell feature ${reverse ? 'reverse' : ''}`}>
        <div className="feature-copy">
          <p className="eyebrow">{signal}</p>
          <h2>{title}</h2>
          <p className="lede">{text}</p>
          <ul>{points.map((point, i) => <li key={point}><b>SYS-{String(i + 1).padStart(2, '0')}</b>{point}</li>)}</ul>
        </div>
        <div className="feature-shots">{shots.map(([src, alt, label]) => <Shot key={src} src={src} alt={alt} label={label} />)}</div>
      </div>
    </section>
  )
}
